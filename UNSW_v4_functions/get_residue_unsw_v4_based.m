...
% **********************************************************
function residue = get_residue_unsw_v4_based(previous_orig_rgb, ref , tgt, ModelID, MAX_NUM_MODELS, set_name, residue_dir, imgNo)
...
%     file_model_name = [mdl_dir '\' set_name '_transform_list_' '_model_id_' num2str(ModelID) '_num_models_' num2str(MAX_NUM_MODELS) '.mat'];
    ...
    [tformList] = enc_model_v4(previous_orig_rgb, tgt, pix_min, pix_max, ModelID, MAX_NUM_MODELS, file_model_name);

    % determine quadtree and perform warping with models
    [pred, Index_Map, QTree_Depth, SMALLEST_BLK_SIZE] = enc_pred_warp_v4(ref, tgt, pix_min, pix_max, tformList, inFlags);
    ...
    writeQT_toBitstream_v4(QTree_Depth, SMALLEST_BLK_SIZE, Index_Map, qtBitStream_name);
    ...
    [residue] = enc_sub_pred_res_v4(pix_min, pix_max, pred, tgt, Index_Map, inFlags, UP_SHIFT);
...
function [tformList] = enc_model_v4(in_ref, in_tgt, pix_min, pix_max, ModelID, MAX_NUM_MODELS, file_model_name)
...
%         
        ...
        [tformList, tformSpatialBoundsList, ListSize] = getTformList_v4(featrs_ref, pnts_ref, featrs_tgt, pnts_tgt, ref, tgt, ModelID, MAX_NUM_MODELS);
    end

end
%% perform wapring of ref image
function [pred, Index_Map, QTree_Depth, SMALLEST_BLK_SIZE] = enc_pred_warp_v4(in_ref, in_tgt, pix_min, pix_max, tformList, inFlags)
    ...
    ref2 = upSample2_v4(ref, pix_min, pix_max);
    ...
    for i = 1:ListSize
        ...
        ref_w = downSample2_v4(ref2_w, pix_max, pix_min);
        ...
        pred = smooth_roll_off_v4(ref_w, msk_w(:,:,1));
    ...
    for QT_Level = 1:QTree_Depth
        ...
        scoreMetric = getBlockErrorSum_v4(SMALLEST_BLK_SIZE, blk_size, err, size(tgt, 1), size(tgt, 2), ListSize, DWT_METRIC_FLAG);
...
%         title(['Quadtree Level ', num2str(QT_Level-1)]);
    ...
    for i = 1:ListSize
        ...
        if i >= 1 && NewModelFlag == 1
            ...
            tform2 = optimizeTrans_v4(tform2, ref2, tgt, msk_combined, synthGainSqrt, pix_min, pix_max);
        ...
        ref2_w = smooth_roll_off_v4(ref2_w, msk2_w);
    ...
    pfm_folder = 'data\';
    if(~exist(pfm_folder, 'dir')), mkdir(pfm_folder); end
    out_residue_name = [pfm_folder 'pred.pfm'];
    write_pfm(single(pred), out_residue_name, 'l', 0);
end

%% subtract prediction from target to generate residue
function [residue] = enc_sub_pred_res_v4(pix_min, pix_max, pred, tgt, Index_Map, inFlags, UP_SHIFT)
...
function [smth] = smooth_roll_off_v4(ref_w, in_msk_w)
...
function up2 = upSample2_v4(in_ref, pix_min, pix_max)
...
function down2 = downSample2_v4(in_ref, pix_max, pix_min)
...
function [tformList, tformSpatialBoundsList, ListSize] = getTformList_v4(featrs_ref, pnts_ref, featrs_tgt, pnts_tgt, ref, tgt, ModelID, max_num_models)
...
function p = multivariateGaussian_v4(X, mu, Sigma2)
...
%% get summation of error for block_size for each model
function scoreMetric = getBlockErrorSum_v4(SMALLEST_BLK_SIZE, blk_size, err, nrows, ncols, ListSize, DWT_METRIC_FLAG)
    r = 0;
    ...
    for row_start = 1:blk_size:nrows
        ...
        for col_start = 1:blk_size:ncols
            ...
            if DWT_METRIC_FLAG == 1
                ...
                score = zeros(1,ListSize+1);
                MSK = ones(8,8); 
                MSK(1:4,1:4) = 0;

                for r_start = row_start:8:row_end
                    for c_start = col_start:8:col_end
                        r_end = min(r_start + 7, row_end);
                        c_end = min(c_start + 7, col_end);

                        for lst=1:size(err,3)
                            se(:,:) = double(abs(dct2(err(r_start:r_end, c_start:c_end, lst))) .* MSK);
                            se(1,1) = 0;
                            score(lst) = score(lst) + sum(sum(se(:,:)));
                        end
                    end
                end

                %scoreMetric(r,c, :) = var(err(row_start:row_end, col_start:col_end, :), 1, [1 2]) * (blk_size/SMALLEST_BLK_SIZE)^2;
                scoreMetric(r,c, :) = score(:);
            end
        end
    end
end
...
function tform2 = optimizeTrans_v4(tform2, ref2, tgt, msk_mdl, synthGainSqrt, pix_min, pix_max)
...
function writeQT_toBitstream_v4(QTree_Depth, SMALLEST_BLK_SIZE, Index_Map, streamFileName)
