...
% **********************************************************
function dec_image = inverse_residue_unsw_v4_based(set_name, residue_folder, ref_recon_image, target_image, dec_resid_name, ModelID, MAX_NUM_MODELS, QTree_Depth, SMALLEST_BLK_SIZE, imgNo)
...
%     dec_resid_name = [out_folder 'dec_residue.ppm'];
    ...
    [qt_bits, pred, I_Blk_Map] = dec_pred_warp_v4(pix_min, pix_max, ref, size(tgt), tformList, qtBitStream_name, QTree_Depth, SMALLEST_BLK_SIZE, inFlags);
    ...
    [dec_tgt] = dec_add_pred_res_v4(pix_min, pix_max, pred, dec_residue, I_Blk_Map, inFlags, UP_SHIFT);
...
function [bpp_list, psnr_list] = dec_inter_rd_v4(KDU_DIR, dec_layer_min, dec_layer_max, pix_min, pix_max, ref, tgt, UP_SHIFT, tformList, j2k_out, dec_name, qtBitStream_name, QTree_Depth, SMALLEST_BLK_SIZE)
    
    % dec quadtree and read models and perform warping to determine
    % warped prediction
    [qt_bits, pred, ~] = dec_pred_warp_v4(pix_min, pix_max, ref, size(tgt), tformList, qtBitStream_name, QTree_Depth, SMALLEST_BLK_SIZE, UP_SHIFT);
    ...
    for layerCnt = dec_layer_min:dec_layer_max
        ...
        [dec_tgt] = dec_add_pred_res_v4(pix_min, pix_max, pred, dec_residue, UP_SHIFT);
...
function [qt_bits, pred, I_Blk_Map] = dec_pred_warp_v4(pix_min, pix_max, ref, tgtSize, tformList, qtBitStream_name, QTree_Depth, SMALLEST_BLK_SIZE, inFlags)
    
    REPLACE_INTRA_DPCM = inFlags.replace_intra_with_dpcm;
    COLOR_CONV_FLAG = inFlags.color_conv_flag;
    DWT_METRIC_FLAG = 0;

    [Dec_Index_Map, qt_bits] = readQT_fromBitstream_v4(QTree_Depth, SMALLEST_BLK_SIZE, tgtSize(2), tgtSize(1), qtBitStream_name);
...
function [dec_tgt] = dec_add_pred_res_v4(pix_min, pix_max, pred, dec_residue, I_Blk_Map, inFlags, UP_SHIFT)
    REPLACE_INTRA_DPCM = inFlags.replace_intra_with_dpcm;
    COLOR_CONV_FLAG = inFlags.color_conv_flag;
...
function [bpp_list, psnr_list] = decode_intra_rd_v4(KDU_DIR, UP_SHIFT, dec_layer_min, dec_layer_max, pix_min, pix_max, tgt, j2k_out, dec_name)
    bpp_list = zeros(1, (dec_layer_max-dec_layer_min+1));
    psnr_list = zeros(1, (dec_layer_max-dec_layer_min+1));
...
function [Index_Map, qt_bits] = readQT_fromBitstream_v4(QTree_Depth, SMALLEST_BLK_SIZE, imgW, imgH, streamFileName)
