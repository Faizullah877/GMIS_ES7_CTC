function report = encode_decode_SJU_ARCH_JPEG2000(input)
report = struct();
...
mother_image_compressed_file = fullfile(input.compressed_folder, sprintf("%s.j2c", image_name));
%encoding
command = sprintf("%s -i %s -o %s Qfactor=%d", encoder_exe, mother_image_file, mother_image_compressed_file, q_v);
...
command = sprintf("%s -i %s -o %s", decoder_exe, mother_image_compressed_file, decoded_image_ppm);
tDstartimg = tic;
[status, ~] = system(command);
if(status)
...
for imgNo = 2: no_of_images
    ...
    j2c_image_file = fullfile(input.compressed_folder, sprintf("%s_residue.j2c", image_name));

    %encoding
    command = sprintf("%s -i %s -o %s Qfactor=%d", encoder_exe, residue_tmp_file, j2c_image_file, q_v);
    ...
    command = sprintf("%s -i %s -o %s", decoder_exe, j2c_image_file, decoded_residue_image_ppm);
    tDstartimg = tic;
    [status, ~] = system(command);
    if(status)
        fprintf("Failed to decompress [%s] with [JPEG 2000] codec.\n", jpg_image_file);
    ...
    psnrv = psnr(rec_next_image, Orig_IMG);
